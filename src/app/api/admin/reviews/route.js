...
export async function POST(request) {
  ...
  if (!body || !body.name || !body.comment || !rating || rating < 1 || rating > 5) {
    return NextResponse.json({ error: "Name, rating and comment are required" }, { status: 400 });
  }

  const review = {
    id: randomUUID(),
    packageSlug: body.packageSlug ? String(body.packageSlug).trim() : "",
  ...
  if (review.packageSlug) revalidatePath(`/packages/${review.packageSlug}`);
